      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header">
              <a class="btn btn-primary btn-round text-white pull-right" href="{{route('order.create')}}">New Order</a>
          ...
          <div class="card-body">
            ...
            <table id="datatable" class="table table-striped table-bordered" cellspacing="0" width="100%">
              ...
              @foreach($orders as $order)
                <tr>
                  <td><a href="{{route('order.edit', [ 'order_id' => $order->id])}}">{{$order->id}}</a></td>
                  <td>{{$order->user->name}}</td>
                  <td>{{$order->user->email}}</td>
                  <td>@foreach($order->product as $product)
                  @if($product->pivot->quantity != null)
                  {{$product->name}}: {{$product->pivot->quantity}}<br>
                  @endif
                  @endforeach</td>
                  <td>${{$order->total}}</td>
                  <td> <form method="POST" action="{{route('order.action', ['order_id' => $order->id])}}" autocomplete="off"
            enctype="multipart/form-data">
              ...
              <div class="card-footer ">
                <button type="submit" class="btn btn-success btn-icon btn-sm"><i class="now-ui-icons ui-2_settings-90"></i>
</button>
              </div>
            </form></td>
                </tr>
